...
function initializePairs() {
  ...
  totalPairs = availablePairs.length; // 全ペア数を設定
}
...
function updateComparison() {
  ...
  const completedPairs = totalPairs - availablePairs.length; // 処理済みペア数
  const progressPercent = Math.floor((completedPairs / totalPairs) * 100);
  progressText.textContent = `進行度: ${progressPercent}%`;
...
function showResults() {
  ...
  rankRows.forEach((row, rowIndex) => {
    ...
    row.forEach((character, index) => {
      ...
      // ランキング順位によって画像サイズを変更
      const rankPosition = rowIndex * 3 + index + 1;
      imgElement.classList.add(`rank-${rankPosition}`);

      // 名前表示部分
      let nameElement = document.createElement("div");
      nameElement.textContent = `${rankPosition}位: ${character.name}`;
  ...
  if (results.length > 10) {
    ...
    for (let i = 10; i < results.length; i++) {
      ...
      listItem.textContent = `${i + 1}位: ${results[i].name}`;
